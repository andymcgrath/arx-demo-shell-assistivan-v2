...
import { SfButton, SfPrimaryButton, FieldRow, SectionHeader, SfSelect } from "../components/SfPrimitives";

/** Fields that don't depend on the admin-designated program name. */
const GENERAL_FIELDS: { label: string; value: string }[] = [
  { label: "Offerings", value: "Enrollment;Appeal;Copay;Benefits Investigation;Prior Authorization" },
  {
    label: "Service Types",
    value:
      "Adherence;Appeals;Copay;Escalation;Onboarding;Orders;Reenrollment;Reimbursement;Replacement;Reverification;Benefits Investigation;Bridge Claims;Documentation Only;Prior Authorization;Product Training;Quick Start",
  },
  { label: "Access Pathway", value: "PAP" },
  { label: "Quick Start Description", value: "Something here." },
  { label: "ARX ID Active", value: "✓" },
];
...
const TEAM_FIELDS: { label: string; value: string }[] = [
  { label: "Field Team Exists", value: "Yes" },
  { label: "Number Of Regions", value: "4" },
  { label: "Districts Per Region", value: "4" },
  { label: "Field Rep Types", value: "12" },
  {
  ...
  { label: "Territories Driven By", value: "Zip" },
];
        ...
        <div className="p-5 max-w-4xl">
          {activeTab !== "general" ? (
            ...
            <div className="border border-[#dddbda] rounded overflow-hidden">
              <SectionHeader title="General" collapsed={false} onToggle={() => {}} />
              <div className="grid grid-cols-2">
                {/* Identity fields (Profile Name, Product Family, ARX ID Code,
                    Brand) are governed by the admin-designated program name,
                    not hardcoded — see deriveArxIdCode()/drugName above. */}
                <FieldRow label="Profile Name" value={drugName} />
                <FieldRow label="Product Family" value={drugName} />
                {GENERAL_FIELDS.slice(0, 4).map((f) => (
                  <FieldRow key={f.label} label={f.label} value={f.value} />
                ))}
                <FieldRow label="ARX ID Code" value={arxIdCode} />
                {GENERAL_FIELDS.slice(4).map((f) => (
                  <FieldRow key={f.label} label={f.label} value={f.value} />
                ))}
                <FieldRow label="Brand" value={drugName} />
              </div>

              <SectionHeader title="Team Structure" collapsed={false} onToggle={() => {}} />
              <div className="grid grid-cols-2">
                {TEAM_FIELDS.map((f) => (
                  <FieldRow key={f.label} label={f.label} value={f.value} />
                ))}
              ...
              <div className="grid grid-cols-2">
                <FieldRow label="Include Welcome Kit">
                  {editing ? (
                    ...
                    <span className="text-[13px] text-[#3e3e3c]">{profile.includeWelcomeKit}</span>
                  )}
                </FieldRow>
                <FieldRow label="Hub Does Welcome Kit">
                  {editing ? (
                    ...
                    <span className="text-[13px] text-[#3e3e3c]">{profile.hubDoesWelcomeKit}</span>
                  )}
                </FieldRow>
                <FieldRow label="Distribution Method">
                  {editing ? (
                    ...
                    <span className="text-[13px] text-[#3e3e3c]">{profile.distributionMethod}</span>
                  )}
                </FieldRow>
                <FieldRow label="Distribution Timing">
                  {editing ? (
                    ...
                    <span className="text-[13px] text-[#3e3e3c]">{profile.distributionTiming}</span>
                  )}
                </FieldRow>
                <FieldRow label="Include Welcome Call">
                  {editing ? (
                    ...
                    <span className="text-[13px] text-[#3e3e3c]">{profile.includeWelcomeCall}</span>
                  )}
                </FieldRow>
                <FieldRow label="Welcome Call Timing">
                  {editing ? (
                    ...
                    <span className="text-[13px] text-[#3e3e3c]">{profile.welcomeCallTiming}</span>
                  )}
                </FieldRow>
                <FieldRow label="Welcome Call Owner">
                  {editing ? (
                    ...
                    <span className="text-[13px] text-[#3e3e3c]">{profile.welcomeCallOwner}</span>
                  )}
                </FieldRow>
              </div>
            </div>
