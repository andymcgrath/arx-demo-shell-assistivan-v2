...
function deriveKeanuStatus(workflowData: WorkflowData): PatientStatus | null {
  ...
  // straight into PA questions (no email/login hop, unlike WF1). Neither
  // this nor PA submission itself waits on consent (see
  // coaDtp.ts/coaCopay.ts's SUBMIT_PA handlers, and the isCoaFlow-scoped
  // auto-trigger in crm/pages/Index.tsx, which now auto-submits the instant
  // BI completes) — reachable while consentDot above is still "pending".
...
export default function ProviderPortal() {
  ...
  useEffect(() => {
    ...
    // not just an optimization. Neither BI nor PA submission wait on patient
    // consent anymore (see coaDtp.ts/coaCopay.ts's RUN_BI/SUBMIT_PA
    // handlers) — PA is now auto-submitted the instant BI completes (see
    // crm/pages/Index.tsx's isCoaFlow-scoped SUBMIT_PA effect), so this can
    // (and normally will) fire well before the patient has done anything.
